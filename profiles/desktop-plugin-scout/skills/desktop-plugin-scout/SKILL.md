...
6. Validate the committed retrospective cutoff reconstruction offline with `node scripts/refresh-desktop-plugins.js --validate`. Detect recorded evidence-path drift at current heads with `GITHUB_TOKEN="$(gh auth token)" node scripts/refresh-desktop-plugins.js --check`. Run without `--check` only after reviewing the revisions to advance. The check does not discover new entrypoint paths. Git history preserves prior retrospective cutoff reconstructions.
