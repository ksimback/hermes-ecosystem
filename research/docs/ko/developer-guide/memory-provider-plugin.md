...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/developer-guide/memory-provider-plugin
...
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/docs/ko/docs/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `hermes plugins`.
