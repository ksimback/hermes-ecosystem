...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/developer-guide/programmatic-integration
...
Full lifecycle, event bridge, and approval flow: [ACP Internals](/docs/ko/developer-guide/acp-internals).
...
Setup, headers (`X-Hermes-Session-Id`, `X-Hermes-Session-Key`), and frontend wiring: [API Server](/docs/ko/user-guide/features/api-server).
...
-   **You want a Python in-process embed without a subprocess** → import `run_agent.AIAgent` directly. See [Agent Loop](/docs/ko/developer-guide/agent-loop).
