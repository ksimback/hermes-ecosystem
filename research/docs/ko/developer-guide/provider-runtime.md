...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/developer-guide/provider-runtime
...
If you are trying to add a new first-class inference provider, read [Adding Providers](/docs/ko/developer-guide/adding-providers) and the [Model Provider Plugin guide](/docs/ko/developer-guide/model-provider-plugin) alongside this page.
...
Cron jobs **do** support fallback: `run_job()` reads `fallback_providers` (or legacy `fallback_model`) from `config.yaml` and passes it to `AIAgent(fallback_model=...)`, matching the gateway's `_load_fallback_model()` pattern. See [Cron Internals](/docs/ko/developer-guide/cron-internals).
...
-   [Context Compression & Prompt Caching](/docs/ko/developer-guide/context-compression-and-caching)
