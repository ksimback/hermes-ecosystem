...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/guides/build-a-hermes-plugin
...
[Model Provider Plugins](/docs/ko/docs/developer-guide/model-provider-plugin)

A **gateway channel** (Discord/Telegram/IRC/Teams/etc.)

[Adding Platform Adapters](/docs/ko/docs/developer-guide/adding-platform-adapters)

A **memory backend** (Honcho/Mem0/Supermemory/etc.)

[Memory Provider Plugins](/docs/ko/docs/developer-guide/memory-provider-plugin)

A **context-compression engine**

[Context Engine Plugins](/docs/ko/docs/developer-guide/context-engine-plugin)

An **image-generation backend**

[Image Generation Provider Plugins](/docs/ko/docs/developer-guide/image-gen-provider-plugin)

A **video-generation backend**

[Video Generation Provider Plugins](/docs/ko/docs/developer-guide/video-gen-provider-plugin)

A **TTS backend** (any CLI — Piper, VoxCPM, Kokoro, voice cloning, …)

[TTS custom command providers](/docs/ko/docs/user-guide/features/tts#custom-command-providers) — config-driven, no Python needed

An **STT backend** (custom whisper / ASR CLI)

[Voice Message Transcription](/docs/ko/docs/user-guide/features/tts#voice-message-transcription-stt) — set `HERMES_LOCAL_STT_COMMAND` to a shell template

**External tools via MCP** (filesystem, GitHub, Linear, any MCP server)

[MCP](/docs/ko/docs/user-guide/features/mcp) — declare `mcp_servers.<name>` in `config.yaml`

**Gateway event hooks** (fire on startup, session events, commands)

[Event Hooks](/docs/ko/docs/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.hermes/hooks/<name>/`

**Shell hooks** (run a shell command on events)

[Shell Hooks](/docs/ko/docs/user-guide/features/hooks#shell-hooks) — declare under `hooks:` in `config.yaml`

**Additional skill sources** (custom GitHub repos, private skill indexes)

[Skills](/docs/ko/docs/user-guide/features/skills) — `hermes skills tap add <repo>` · [Publishing a tap](/docs/ko/docs/user-guide/features/skills#publishing-a-custom-skill-tap)

A first-class **core** inference provider (not a plugin)

[Adding Providers](/docs/ko/docs/developer-guide/adding-providers)

See the full [Pluggable interfaces table](/docs/ko/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each) for a consolidated view of every extension surface including config-driven (TTS, STT, MCP, shell hooks) and drop-in directory (gateway hooks) styles.
...
Each hook is documented in full on the **[Event Hooks reference](/docs/ko/docs/user-guide/features/hooks#plugin-hooks)** — callback signatures, parameter tables, exactly when each fires, and examples. Here's the summary:
...
[`pre_tool_call`](/docs/ko/docs/user-guide/features/hooks#pre_tool_call)
...
[`post_tool_call`](/docs/ko/docs/user-guide/features/hooks#post_tool_call)
...
[`pre_llm_call`](/docs/ko/docs/user-guide/features/hooks#pre_llm_call)
...
[`post_llm_call`](/docs/ko/docs/user-guide/features/hooks#post_llm_call)
...
[`on_session_start`](/docs/ko/docs/user-guide/features/hooks#on_session_start)
...
[`on_session_end`](/docs/ko/docs/user-guide/features/hooks#on_session_end)
...
[`on_session_finalize`](/docs/ko/docs/user-guide/features/hooks#on_session_finalize)
...
[`on_session_reset`](/docs/ko/docs/user-guide/features/hooks#on_session_reset)
...
**Memory provider plugins** use a convention-based approach instead: add a `register_cli(subparser)` function to your plugin's `cli.py` file. The memory plugin discovery system finds it automatically — no `ctx.register_cli_command()` call needed. See the [Memory Provider Plugin guide](/docs/ko/docs/developer-guide/memory-provider-plugin#adding-cli-commands) for details.
...
**Full guide:** [Model Provider Plugins](/docs/ko/docs/developer-guide/model-provider-plugin) — field reference, overridable hooks (`prepare_messages`, `build_extra_body`, `build_api_kwargs_extras`, `fetch_models`), api\_mode selection, auth types, testing.
...
**Full guide:** [Adding Platform Adapters](/docs/ko/docs/developer-guide/adding-platform-adapters) — complete `BasePlatformAdapter` contract, message routing, auth gating, setup wizard integration. Look at `plugins/platforms/irc/` for a stdlib-only working example.
...
**Full guide:** [Memory Provider Plugins](/docs/ko/docs/developer-guide/memory-provider-plugin) — full `MemoryProvider` ABC, threading contract, profile isolation, CLI command registration via `cli.py`.
...
**Full guide:** [Context Engine Plugins](/docs/ko/docs/developer-guide/context-engine-plugin).
...
**Full guide:** [Image Generation Provider Plugins](/docs/ko/docs/developer-guide/image-gen-provider-plugin) — full `ImageGenProvider` ABC, `list_models()` / `get_setup_schema()` metadata, `success_response()`/`error_response()` helpers, base64 vs URL output, user overrides, pip distribution.
...
Hermes also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/docs/ko/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.
...
Hermes connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/docs/ko/docs/user-guide/features/mcp).
...
**Full guide:** [Gateway Event Hooks](/docs/ko/docs/user-guide/features/hooks#gateway-event-hooks).
...
**Full guide:** [Shell Hooks](/docs/ko/docs/user-guide/features/hooks#shell-hooks).
...
**Full guides:** [Skills Hub](/docs/ko/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/ko/docs/user-guide/features/skills#publishing-a-custom-skill-tap) (repo layout, minimal example, non-default paths, trust levels).
...
**Full guides:** [TTS custom command providers](/docs/ko/docs/user-guide/features/tts#custom-command-providers) · [STT](/docs/ko/docs/user-guide/features/tts#voice-message-transcription-stt).
...
See the [Nix Setup guide](/docs/ko/docs/getting-started/nix-setup#plugins) for complete documentation including overlay usage and collision checking.
