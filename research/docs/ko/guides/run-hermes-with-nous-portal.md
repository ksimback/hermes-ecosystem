...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/guides/run-hermes-with-nous-portal

This guide walks you through running Hermes Agent on a [Nous Portal](https://portal.nousresearch.com) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [Nous Portal integration page](/docs/ko/docs/integrations/nous-portal). This page is the task script.

## Prerequisites

-   Hermes Agent installed ([Quickstart](/docs/ko/docs/getting-started/quickstart))
-   A web browser on the machine you're setting up (or SSH port forwarding — see [OAuth over SSH](/docs/ko/docs/guides/oauth-over-ssh))
-   About 5 minutes
...
See [OAuth over SSH / Remote Hosts](/docs/ko/docs/guides/oauth-over-ssh) for the full walkthrough including ProxyJump chains, mosh/tmux, and ControlMaster gotchas.
...
Because the Tool Gateway includes OpenAI TTS, [voice mode](/docs/ko/docs/user-guide/features/voice-mode) works without a separate OpenAI key:
...
The Portal subscription works for [cron jobs](/docs/ko/docs/user-guide/features/cron) and [batch processing](/docs/ko/docs/user-guide/features/batch-processing) the same way it works for interactive chat — the OAuth refresh token is reused automatically. No additional setup; just schedule cron jobs and they'll bill against your subscription.
...
If you use [Hermes profiles](/docs/ko/docs/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.
...
If your browser doesn't open or the callback fails, you're likely on a remote/headless host — see [OAuth over SSH](/docs/ko/docs/guides/oauth-over-ssh) for the port-forwarding and manual-paste workarounds.
...
-   **[Nous Portal integration page](/docs/ko/docs/integrations/nous-portal)** — Overview of what's in the subscription
-   **[Tool Gateway](/docs/ko/docs/user-guide/features/tool-gateway)** — Full details on every gateway-routed tool
-   **[Subscription proxy](/docs/ko/docs/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Hermes tools
-   **[Voice mode](/docs/ko/docs/user-guide/features/voice-mode)** — Set up voice conversations on the Portal subscription
-   **[OAuth over SSH](/docs/ko/docs/guides/oauth-over-ssh)** — Remote / headless login patterns
-   **[Profiles](/docs/ko/docs/user-guide/profiles)** — Share one Portal login across multiple Hermes configurations
