...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/guides/webhook-github-pr-review
...
If you don't have a public URL or just want to get started quickly, check out [Build a GitHub PR Review Agent](/docs/ko/guides/github-pr-review-agent) — uses cron jobs to poll for PRs on a schedule, works behind NAT and firewalls.

Reference docs

For the full webhook platform reference (all config options, delivery types, dynamic subscriptions, security model) see [Webhooks](/docs/ko/docs/user-guide/messaging/webhooks).
...
Load a [Hermes skill](/docs/ko/docs/user-guide/features/skills) to give the agent a consistent review persona. Add `skills` to your route inside `platforms.webhook.extra.routes` in `config.yaml`:
...
-   **[Cron-Based PR Reviews](/docs/ko/guides/github-pr-review-agent)** — poll for PRs on a schedule, no public endpoint needed
-   **[Webhook Reference](/docs/ko/docs/user-guide/messaging/webhooks)** — full config reference for the webhook platform
-   **[Build a Plugin](/docs/ko/docs/guides/build-a-hermes-plugin)** — package review logic into a shareable plugin
-   **[Profiles](/docs/ko/docs/user-guide/profiles)** — run a dedicated reviewer profile with its own memory and config
