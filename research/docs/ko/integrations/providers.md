...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/integrations/providers
...
`hermes model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](/docs/ko/guides/xai-grok-oauth)
...
For the official API-key path, see the dedicated [Google Gemini guide](/docs/ko/docs/guides/google-gemini).
...
**For full details:** see the dedicated [Nous Portal integration page](/docs/ko/docs/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Hermes Agent with Nous Portal guide](/docs/ko/docs/guides/run-hermes-with-nous-portal).
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `hermes tools` → Voice & TTS, or see the [Voice & TTS](/docs/ko/user-guide/features/tts#text-to-speech) page for config.
...
See the [AWS Bedrock guide](/docs/ko/docs/guides/aws-bedrock) for a walkthrough of IAM setup, region selection, and cross-region inference.
...
`minimax-oauth` uses MiniMax's consumer-facing portal with OAuth login — no billing setup required. The `minimax` and `minimax-cn` providers use `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY` — for programmatic access. See the [MiniMax OAuth guide](/docs/ko/docs/guides/minimax-oauth) for a full walkthrough.
...
2.  **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/ko/docs/user-guide/configuration#context-compression) in Hermes.
...
-   To use the Pareto Code router for a specific **auxiliary task** (compression, vision, etc.) instead of the main agent, set `extra_body.plugins` under that task — see [Auxiliary Models → OpenRouter routing & Pareto Code for auxiliary tasks](/docs/ko/docs/user-guide/configuration#openrouter-routing--pareto-code-for-auxiliary-tasks).
...
Fallback is configured exclusively through `config.yaml` — or interactively via `hermes fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/ko/docs/user-guide/features/fallback-providers).

* * *

## See Also

-   [Configuration](/docs/ko/docs/user-guide/configuration) — General configuration (directory structure, config precedence, terminal backends, memory, compression, and more)
-   [Environment Variables](/docs/ko/docs/reference/environment-variables) — Complete reference of all environment variables
