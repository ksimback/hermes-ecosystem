...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/reference/environment-variables
...
Sovereign-cloud authority override (e.g. `https://login.microsoftonline.us` for Azure Government). See [Azure Foundry guide](/docs/ko/docs/guides/azure-foundry#sovereign-clouds-government-china)
...
Environment variables for the bundled [`observability/langfuse`](/docs/ko/docs/user-guide/features/built-in-plugins#observabilitylangfuse) plugin. Set these in `~/.hermes/.env`. The plugin must also be enabled (`hermes plugins enable observability/langfuse`, or check the box in `hermes plugins`) before any of these take effect.
...
These variables configure the [Tool Gateway](/docs/ko/docs/user-guide/features/tool-gateway) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `hermes model` or `hermes tools`.
...
Allow the bot to ping `@everyone`/`@here` (default: `false`). See [Mention Control](/docs/ko/user-guide/messaging/discord#mention-control).
...
`none` (default) / `mentions` / `all` — accept inbound messages from other bots. See [bot-to-bot messaging](/docs/ko/user-guide/messaging/feishu#bot-to-bot-messaging)
...
URL of a remote Hermes API server to forward messages to ([proxy mode](/docs/ko/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos)). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Also configurable via `gateway.proxy_url` in `config.yaml`.
...
App-only credentials for the Microsoft Graph REST client used by the upcoming Teams meeting summary pipeline. See [Register a Microsoft Graph application](/docs/ko/docs/guides/microsoft-graph-app-registration) for the Azure portal walkthrough and the exact API permissions required.
...
Inbound change-notification listener for Graph events (Teams meetings, calendar, chat, etc.). See [Microsoft Graph Webhook Listener](/docs/ko/docs/user-guide/messaging/msgraph-webhook) for setup and security hardening.
...
Only used when the [`teams_pipeline` plugin](/docs/ko/docs/user-guide/messaging/msgraph-webhook) is enabled. Settings are also configurable under `platforms.teams.extra` in `config.yaml` — env vars take priority when both are set. See [Microsoft Teams → Meeting Summary Delivery](/docs/ko/docs/user-guide/messaging/teams#meeting-summary-delivery-teams-meeting-pipeline).
...
Used by the bundled LINE platform plugin (`plugins/platforms/line/`). See [Messaging Gateway → LINE](/docs/ko/docs/user-guide/messaging/line) for full setup.
...
See [the ntfy messaging guide](/docs/ko/docs/user-guide/messaging/ntfy) — particularly the **identity model** section — before deploying with untrusted topics.
...
When [Codex app-server runtime](/docs/ko/user-guide/features/codex-app-server-runtime) is enabled, override the directory Codex CLI reads its config + auth from (default: `~/.codex`). Hermes' migration writes the managed block to `<CODEX_HOME>/config.toml`.
...
Launch the [TUI](/docs/ko/user-guide/tui) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`.
...
See [Fallback Providers](/docs/ko/docs/user-guide/features/fallback-providers) for full details.
