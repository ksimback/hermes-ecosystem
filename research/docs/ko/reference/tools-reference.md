...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/reference/tools-reference
...
In addition to built-in tools, Hermes can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp_<server>_` (e.g., `mcp_github_create_issue` for the `github` MCP server). See [MCP Integration](/docs/ko/docs/user-guide/features/mcp) for configuration.
...
**Honcho tools** (`honcho_profile`, `honcho_search`, `honcho_context`, `honcho_reasoning`, `honcho_conclude`) are no longer built-in. They are available via the Honcho memory provider plugin at `plugins/memory/honcho/`. See [Memory Providers](/docs/ko/user-guide/features/memory-providers) for installation and usage.
