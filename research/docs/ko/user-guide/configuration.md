...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/configuration
...
For AI provider setup (OpenRouter, Anthropic, Copilot, custom endpoints, self-hosted LLMs, fallback models, etc.), see [AI Providers](/docs/ko/docs/integrations/providers).
...
See [Code Execution](/docs/ko/user-guide/features/code-execution) and the [Terminal section of the README](/docs/ko/user-guide/features/tools) for details on each backend.
...
For details on declaring config settings in your own skills, see [Creating Skills — Config Settings](/docs/ko/docs/developer-guide/creating-skills#config-settings-configyaml).
...
The context engine controls how conversations are managed when approaching the model's token limit. The built-in `compressor` engine uses lossy summarization (see [Context Compression](/docs/ko/docs/developer-guide/context-compression-and-caching)). Plugin engines can replace it with alternative strategies.
...
See [Memory Providers](/docs/ko/docs/user-guide/features/memory-providers) for the analogous single-select system for memory plugins.
...
`agent.api_max_retries` controls how many times Hermes retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/docs/ko/docs/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.
...
Options: `fill_first` (default), `round_robin`, `least_used`, `random`. See [Credential Pools](/docs/ko/docs/user-guide/features/credential-pools) for full documentation.
...
Available providers for auxiliary tasks: `auto`, `main`, plus any provider in the [provider registry](/docs/ko/docs/reference/environment-variables) — `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `google-gemini-cli`, `qwen-oauth`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `alibaba`, `bedrock`, `huggingface`, `arcee`, `xiaomi`, `kilocode`, `opencode-zen`, `opencode-go`, `ai-gateway`, `azure-foundry` — or any named custom provider from your `custom_providers` list (e.g. `provider: "beans"`).

MiniMax OAuth

`minimax-oauth` logs in via browser OAuth (no API key needed). Run `hermes model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](/docs/ko/guides/minimax-oauth).

xAI Grok OAuth

`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `hermes model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](/docs/ko/guides/xai-grok-oauth), and if Hermes is on a remote host see [OAuth over SSH / Remote Hosts](/docs/ko/guides/oauth-over-ssh).

`"main"` is for auxiliary tasks only

The `"main"` provider option means "use whatever provider my main agent uses" — it's only valid inside `auxiliary:`, `compression:`, and `fallback_model:` configs. It is **not** a valid value for your top-level `model.provider` setting. If you use a custom OpenAI-compatible endpoint, set `provider: custom` in your `model:` section. See [AI Providers](/docs/ko/docs/integrations/providers) for all main model provider options.
...
Run `hermes model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](/docs/ko/guides/minimax-oauth) for the full walkthrough.
...
Use `/voice on` in the CLI to enable microphone mode, `record_key` to start/stop recording, and `/voice tts` to toggle spoken replies. See [Voice Mode](/docs/ko/docs/user-guide/features/voice-mode) for end-to-end setup and platform-specific behavior.
...
For the behavior details and examples, see [Sessions](/docs/ko/docs/user-guide/sessions) and the [Discord guide](/docs/ko/docs/user-guide/messaging/discord).
...
**SearXNG** is a free, self-hosted, privacy-respecting metasearch engine that queries 70+ search engines. No API key needed — just set `SEARXNG_URL` to your instance (e.g., `http://localhost:8080`). SearXNG is search-only; `web_extract` and `web_crawl` require a separate extract provider (set `web.extract_backend`). See the [Web Search setup guide](/docs/ko/docs/user-guide/features/web-search) for Docker setup instructions.
...
See the [browser feature page](/docs/ko/user-guide/features/browser#browser_dialog) for the full dialog workflow.

The browser toolset supports multiple providers. See the [Browser feature page](/docs/ko/docs/user-guide/features/browser) for details on Browserbase, Browser Use, and local Chromium-family CDP setup.
...
Automatic filesystem snapshots before destructive file operations. See the [Checkpoints & Rollback](/docs/ko/docs/user-guide/checkpoints-and-rollback) for details.
...
-   [Personality & SOUL.md](/docs/ko/docs/user-guide/features/personality)
