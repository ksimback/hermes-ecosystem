...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/api-server
...
Or connect Open WebUI, LobeChat, or any other frontend — see the [Open WebUI integration guide](/docs/ko/docs/user-guide/messaging/open-webui) for step-by-step instructions.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/docs/ko/docs/user-guide/profiles) name (or `hermes-agent` for the default profile). Required by most frontends for model discovery.
...
[Open WebUI](/docs/ko/docs/user-guide/messaging/open-webui)
...
To give multiple users their own isolated Hermes instance (separate config, memory, skills), use [profiles](/docs/ko/docs/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Hermes instance. See the [Open WebUI guide](/docs/ko/docs/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
...
See [Matrix Proxy Mode](/docs/ko/docs/user-guide/messaging/matrix#proxy-mode-e2ee-on-macos) for the full setup guide.
