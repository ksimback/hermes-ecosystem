...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/built-in-plugins
...
See the [Plugins](/docs/ko/docs/user-guide/features/plugins) page for the general plugin system, and [Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin) to write your own.
...
Kanban board UI for the multi-agent dispatcher — tasks, comments, fan-out, board switching. See [Kanban Multi-Agent](/docs/ko/user-guide/features/kanban.md).

Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](/docs/ko/user-guide/features/memory-providers) — they're managed through `hermes memory` and `hermes plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
Bundled plugins are written exactly like any other Hermes plugin — see [Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin). The only differences are:
