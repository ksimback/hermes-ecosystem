...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/credential-pools
...
This is different from [fallback providers](/docs/ko/user-guide/features/fallback-providers), which switch to a _different_ provider entirely. Credential pools are same-provider rotation; fallback providers are cross-provider failover. Pools are tried first — if all pool keys are exhausted, _then_ the fallback provider activates.
