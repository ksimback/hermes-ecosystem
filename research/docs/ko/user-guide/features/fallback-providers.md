...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/fallback-providers

Hermes Agent has three layers of resilience that keep your sessions running when providers hit issues:

1.  **[Credential pools](/docs/ko/user-guide/features/credential-pools)** — rotate across multiple API keys for the _same_ provider (tried first)
...
These options apply to `auxiliary:`, `compression:`, and `fallback_model:` configs only — `"main"` is **not** a valid value for your top-level `model.provider`. For custom endpoints, use `provider: custom` in your `model:` section (see [AI Providers](/docs/ko/docs/integrations/providers)).
...
See [Subagent Delegation](/docs/ko/docs/user-guide/features/delegation) for full configuration details.
...
See [Scheduled Tasks (Cron)](/docs/ko/docs/user-guide/features/cron) for full configuration details.
