...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/kanban-tutorial

브라우저에 dashboard를 띄운 상태에서, Hermes Kanban 시스템이 설계된 4가지 대표 사용 사례를 따라가는 walkthrough입니다. 아직 [Kanban 개요](/docs/ko/user-guide/features/kanban)를 읽지 않았다면 먼저 그 문서부터 보세요. 이 튜토리얼은 task, run, assignee, dispatcher의 의미를 이미 안다고 가정합니다.
...
dashboard는 시스템을 지켜보는 **사람인 당신**에게 가장 편한 인터페이스입니다. dispatcher가 spawn하는 agent worker는 dashboard나 CLI를 직접 보지 않습니다. 이들은 전용 `kanban_*` [toolset](/docs/ko/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`)으로 보드를 다룹니다. dashboard, CLI, worker tool은 모두 같은 board별 SQLite DB(기본 board는 `~/.hermes/kanban.db`, 이후 만든 board는 `~/.hermes/kanban/boards/<slug>/kanban.db`)를 통하므로, 어느 쪽에서 바꿔도 보드 상태는 일관됩니다.

이 튜토리얼은 계속 `default` board를 사용합니다. 프로젝트/레포/도메인별로 여러 개의 격리된 queue를 원한다면 개요 문서의 [Boards (멀티 프로젝트)](/docs/ko/user-guide/features/kanban#boards-multi-project)를 보세요. CLI / dashboard / worker 흐름은 똑같고, worker는 물리적으로 다른 board의 task를 볼 수 없습니다.
...
![Kanban board overview](/docs/ko/assets/images/01-board-overview-dd98ac39283d95534d6df59f90df98b0.png)
...
![Board with lanes by profile off](/docs/ko/assets/images/02-board-flat-019a7cf4f943c0ff4d95ffa7d579bf25.png)
...
![Solo dev — completed schema task drawer](/docs/ko/assets/images/03-drawer-schema-task-217f2993df9ebe7c0f2a678df97d934e.png)
...
![Fleet view filtered to transcribe tasks](/docs/ko/assets/images/07-fleet-transcribes-cf7345a3270df63f9336252cada8da61.png)
...
![Pipeline view for a multi-role feature](/docs/ko/assets/images/08-pipeline-auth-2dc4dcc631cf371f649ad70ab1e71611.png)
...
![Implementation task with two runs — blocked then completed](/docs/ko/assets/images/04b-drawer-retry-history-scrolled-356e4195a55572cfc522c5236f02c594.png)
...
![Reviewer&#39;s drawer view of the pipeline](/docs/ko/assets/images/09-drawer-pipeline-review-12ea45e97703570d5e889812347a0778.png)
...
![Circuit breaker — 2 spawn\_failed + 1 gave\_up](/docs/ko/assets/images/11-drawer-gave-up-04cdec442b239748a9904b8651efde1d.png)
...
![Crash and recovery — 1 crashed + 1 completed](/docs/ko/assets/images/06-drawer-crash-recovery-17edcbea131d965bb8dc7283e5622e35.png)
...
![Claimed, in-flight task](/docs/ko/assets/images/10-drawer-in-flight-0b6c924e986c7862439f00d776e432f4.png)
...
-   [Kanban 개요](/docs/ko/user-guide/features/kanban) — 전체 데이터 모델, event vocabulary, CLI reference
