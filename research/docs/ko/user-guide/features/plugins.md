...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/plugins

Hermes has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

If you want to create a custom tool for yourself, your team, or one project, this is usually the right path. The developer guide's [Adding Tools](/docs/ko/docs/developer-guide/adding-tools) page is for built-in Hermes core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.
...
`ctx.register_platform(name, label, adapter_factory, check_fn, ...)` — see [Adding Platform Adapters](/docs/ko/docs/developer-guide/adding-platform-adapters)

Register an image-generation backend

`ctx.register_image_gen_provider(provider)` — see [Image Generation Provider Plugins](/docs/ko/docs/developer-guide/image-gen-provider-plugin)

Register a video-generation backend

`ctx.register_video_gen_provider(provider)` — see [Video Generation Provider Plugins](/docs/ko/docs/developer-guide/video-gen-provider-plugin)

Register a context-compression engine

`ctx.register_context_engine(engine)` — see [Context Engine Plugins](/docs/ko/docs/developer-guide/context-engine-plugin)

Register a memory backend

Subclass `MemoryProvider` in `plugins/memory/<name>/__init__.py` — see [Memory Provider Plugins](/docs/ko/docs/developer-guide/memory-provider-plugin) (uses a separate discovery system)

Run a host-owned LLM call

`ctx.llm.complete(...)` / `ctx.llm.complete_structured(...)` — borrow the user's active model + auth for a one-shot completion with optional JSON schema validation. See [Plugin LLM Access](/docs/ko/docs/developer-guide/plugin-llm-access)

Register an inference backend (LLM provider)

`register_provider(ProviderProfile(...))` in `plugins/model-providers/<name>/__init__.py` — see [Model Provider Plugins](/docs/ko/docs/developer-guide/model-provider-plugin) (uses a separate discovery system)
...
Ships with Hermes — see [Built-in Plugins](/docs/ko/docs/user-guide/features/built-in-plugins)
...
NixOS declarative installs — see [Nix Setup](/docs/ko/docs/getting-started/nix-setup#plugins)
...
Plugins can register callbacks for these lifecycle events. See the **[Event Hooks page](/docs/ko/docs/user-guide/features/hooks#plugin-hooks)** for full details, callback signatures, and examples.
...
Once per turn, before the LLM loop — can return `{"context": "..."}` to [inject context into the user message](/docs/ko/docs/user-guide/features/hooks#pre_llm_call)
...
[`subagent_stop`](/docs/ko/docs/user-guide/features/hooks#subagent_stop)

Once per child after `delegate_task` finishes

[`pre_gateway_dispatch`](/docs/ko/docs/user-guide/features/hooks#pre_gateway_dispatch)
...
[Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin) · [Adding Tools](/docs/ko/docs/developer-guide/adding-tools)

A **lifecycle hook** (pre/post LLM, session start/end, tool filter)

Python plugin — `ctx.register_hook()`

[Hooks reference](/docs/ko/docs/user-guide/features/hooks) · [Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin)

A **slash command** for the CLI / gateway

Python plugin — `ctx.register_command()`

[Build a Hermes Plugin](/docs/ko/docs/guides/build-a-hermes-plugin) · [Extending the CLI](/docs/ko/docs/developer-guide/extending-the-cli)

A **subcommand** for `hermes <thing>`

Python plugin — `ctx.register_cli_command()`

[Extending the CLI](/docs/ko/docs/developer-guide/extending-the-cli)

A bundled **skill** that your plugin ships

Python plugin — `ctx.register_skill()`

[Creating Skills](/docs/ko/docs/developer-guide/creating-skills)
...
**[Model Provider Plugins](/docs/ko/docs/developer-guide/model-provider-plugin)** · [Adding Providers](/docs/ko/docs/developer-guide/adding-providers)
...
[TTS Setup](/docs/ko/docs/user-guide/features/tts#custom-command-providers) · [Python plugin guide](/docs/ko/docs/user-guide/features/tts#python-plugin-providers)

An **STT backend** (custom whisper binary, local ASR CLI)

Config-driven — set `HERMES_LOCAL_STT_COMMAND` env var to a shell template

[Voice Message Transcription (STT)](/docs/ko/docs/user-guide/features/tts#voice-message-transcription-stt)
...
[Skills Hub](/docs/ko/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/ko/docs/user-guide/features/skills#publishing-a-custom-skill-tap)
...
[Event Hooks](/docs/ko/docs/user-guide/features/hooks#gateway-event-hooks)
...
[Shell Hooks](/docs/ko/docs/user-guide/features/hooks#shell-hooks)
...
On NixOS, plugins can be installed declaratively via the module options — no `hermes plugins install` needed. See the **[Nix Setup guide](/docs/ko/docs/getting-started/nix-setup#plugins)** for full details.
...
See the **[full guide](/docs/ko/docs/guides/build-a-hermes-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
