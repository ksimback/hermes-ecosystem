...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/features/web-search
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Tool Gateway](/docs/ko/user-guide/features/tool-gateway)** via managed Firecrawl — no API key needed. New installs can run `hermes setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `hermes tools`.
...
See [Auxiliary Models](/docs/ko/docs/user-guide/configuration#auxiliary-models) for the full reference and per-task override patterns.
