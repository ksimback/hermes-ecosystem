...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/messaging/open-webui
...
To run separate Hermes instances per user — each with their own config, memory, and skills — use [profiles](/docs/ko/docs/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
