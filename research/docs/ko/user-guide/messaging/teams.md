...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/messaging/teams
...
Need meeting summaries from Microsoft Graph events rather than normal bot conversations? Use the dedicated setup page: [Teams Meetings](/docs/ko/docs/user-guide/messaging/teams-meetings).
...
When the [Teams meeting pipeline plugin](/docs/ko/docs/user-guide/messaging/msgraph-webhook) is enabled, this adapter also handles outbound delivery of meeting summaries — one Teams integration surface, not two. After a meeting's transcript is summarized, the writer posts the summary into your chosen Teams target.
...
Requires the [Graph app registration](/docs/ko/docs/guides/microsoft-graph-app-registration) with `ChannelMessage.Send` (channel) or `Chat.ReadWrite.All` (chat) application permissions.
...
-   [Teams Meetings](/docs/ko/docs/user-guide/messaging/teams-meetings)
