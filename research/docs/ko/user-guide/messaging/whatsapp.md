...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/messaging/whatsapp
...
Setting `WHATSAPP_ALLOWED_USERS=*` allows **all** senders (equivalent to `WHATSAPP_ALLOW_ALL_USERS=true`). This is consistent with [Signal group allowlists](/docs/ko/docs/reference/environment-variables). To use the pairing flow instead, remove both variables and rely on the [DM pairing system](/docs/ko/docs/user-guide/security#dm-pairing-system).
...
launchd services don't inherit your shell PATH. Run `hermes gateway install` to re-snapshot your current PATH into the plist, then `hermes gateway start`. See the [Gateway Service docs](/docs/ko/user-guide/messaging/#macos-launchd) for details.
