...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/security
...
For destructive session slash commands (`/clear`, `/new` / `/reset`, `/undo`, `/exit --delete`), the CLI also prompts for confirmation before running them. See [Slash Commands — Confirmation prompts for destructive commands](/docs/ko/reference/slash-commands#confirmation-prompts-for-destructive-commands).
...
See [Website Blocklist](/docs/ko/docs/user-guide/configuration#website-blocklist) in the configuration guide for full details.
