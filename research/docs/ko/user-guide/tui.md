...
**Source:** https://hermes-agent.nousresearch.com/docs/ko/user-guide/tui

The TUI is the modern front-end for Hermes — a terminal UI backed by the same Python runtime as the [Classic CLI](/docs/ko/user-guide/cli). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.
...
The classic CLI remains available as the default. Anything documented in [CLI Interface](/docs/ko/user-guide/cli) — slash commands, quick commands, skill preloading, personalities, multi-line input, interrupts — works in the TUI identically.
...
Same [skins](/docs/ko/user-guide/features/skins) and [personalities](/docs/ko/user-guide/features/personality) apply. Switch mid-session with `/skin ares`, `/personality pirate`, and the UI repaints live. See [Skins & Themes](/docs/ko/user-guide/features/skins) for the full list of customizable keys and which ones apply to classic vs TUI — the TUI honors the banner palette, UI colors, prompt glyph/color, session display, completion menu, selection bg, `tool_prefix`, and `help_header`.
...
Keybindings match the [Classic CLI](/docs/ko/user-guide/cli#keybindings) exactly. The only behavioral differences:
...
Every other slash command (including installed skills, quick commands, and personality toggles) works identically to the classic CLI. See [Slash Commands Reference](/docs/ko/reference/slash-commands).
...
The per-skin status-bar colors and thresholds are shared with the classic CLI — see [Skins](/docs/ko/user-guide/features/skins) for customization.
...
See [Sessions](/docs/ko/user-guide/sessions) for lifecycle, search, compression, and export.
...
The token comes from the gateway's API auth configuration (see [API Server](/docs/ko/user-guide/features/api-server)). When the env var is set, the TUI:
...
This is the same channel the web dashboard's embedded TUI uses (see [Web Dashboard](/docs/ko/user-guide/features/web-dashboard#chat)) — one gateway, many clients.
...
-   [CLI Interface](/docs/ko/user-guide/cli) — full slash command and keybinding reference (shared)
-   [Sessions](/docs/ko/user-guide/sessions) — resume, branch, and history
-   [Skins & Themes](/docs/ko/user-guide/features/skins) — theme the banner, status bar, and overlays
-   [Voice Mode](/docs/ko/user-guide/features/voice-mode) — works in both interfaces
-   [Configuration](/docs/ko/user-guide/configuration) — all config keys
